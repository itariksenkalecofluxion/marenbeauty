...
const written = [];
for (const [slug, group] of SERVICES) {
  const svg = svgFor(slug, group);
  const file = join(outDir, `${slug}.webp`);
  // Sequential on purpose: twenty rasterisations at once is a memory spike for
  // no wall-clock gain on a script nobody waits for.
  await sharp(Buffer.from(svg)).webp({ quality: 86, effort: 6 }).toFile(file);
  written.push(`${slug}.webp`);
}
...
console.log(`\n  ${written.length} placeholder images written to`);
console.log(`  public/images/services/\n`);
