...
exposed as a *semantic* token so a dark theme could be added later without
...
a *seasoning*, not a brand colour. Target: under 2% of any viewport.
...
8. **Text over the aurora background or over a photograph** requires either a
   ...
   shifts as you scroll, so contrast must be verified at *worst case*, not at
...
- Scroll is never hijacked. Only animation *progress* binds to scroll.
